...
if(!isset($_SESSION['loggedin']) || $_SESSION['loggedin']!=true ){
    ...
    if(count(fetchAll($query)) > 0){
        foreach(fetchAll($query) as $row){
            ...
            $query = "INSERT INTO `users_scl` (`userid`, `firstname`, `lastname`,`regdno`, `email`, `password`, `mobile`, `dob`, `class`, `section`, `signuptime`, `usertype`,`Comment`) VALUES (NULL, '$firstname', '$lastname','$regdno', '$email', '$password', '$mobile', '$dob', '$class', '$section', current_timestamp(), '$usertype','$user');";
